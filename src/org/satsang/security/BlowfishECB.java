...
public class BlowfishECB {
	...
	// the boxes, chose new over clone() to make it run on J2ME
	...
	byte[] block_buf = new byte[BLOCKSIZE];
	...
	public void initialize(byte[] key, int ofs, int len) {
		...
		for (i = 0; i < PBOX_ENTRIES;) {
			encryptPrv(this.block_buf, 0, this.block_buf, 0, BLOCKSIZE);
			this.pbox[i++] = BinConverter.byteArrayToInt(this.block_buf, 0);
			this.pbox[i++] = BinConverter.byteArrayToInt(this.block_buf, 4);
		}

		for (i = 0; i < SBOX_ENTRIES;) {
			encryptPrv(this.block_buf, 0, this.block_buf, 0, BLOCKSIZE);
			this.sbox1[i++] = BinConverter.byteArrayToInt(this.block_buf, 0);
			this.sbox1[i++] = BinConverter.byteArrayToInt(this.block_buf, 4);
		}

		for (i = 0; i < SBOX_ENTRIES;) {
			encryptPrv(this.block_buf, 0, this.block_buf, 0, BLOCKSIZE);
			this.sbox2[i++] = BinConverter.byteArrayToInt(this.block_buf, 0);
			this.sbox2[i++] = BinConverter.byteArrayToInt(this.block_buf, 4);
		}

		for (i = 0; i < SBOX_ENTRIES;) {
			encryptPrv(this.block_buf, 0, this.block_buf, 0, BLOCKSIZE);
			this.sbox3[i++] = BinConverter.byteArrayToInt(this.block_buf, 0);
			this.sbox3[i++] = BinConverter.byteArrayToInt(this.block_buf, 4);
		}

		for (i = 0; i < SBOX_ENTRIES;) {
			encryptPrv(this.block_buf, 0, this.block_buf, 0, BLOCKSIZE);
			this.sbox4[i++] = BinConverter.byteArrayToInt(this.block_buf, 0);
			this.sbox4[i++] = BinConverter.byteArrayToInt(this.block_buf, 4);
		}
	...
	/**
	 * To clear data in the boxes before an instance is freed, the actual value
	 * of this call can be argued, yet still nice to have it.
	 */
	public void cleanUp() {
		int i, c;
		...
		for (i = 0; i < SBOX_ENTRIES; i++) {
			this.sbox1[i] = this.sbox2[i] = this.sbox3[i] = this.sbox4[i] = 0;
		}

		for (i = 0, c = this.block_buf.length; i < c; i++) {
			this.block_buf[i] = 0;
		}
	...
	final static byte[] TESTKEY1 = { (byte) 0x1c, (byte) 0x58, (byte) 0x7f, (byte) 0x1c, (byte) 0x13, (byte) 0x92,
			(byte) 0x4f, (byte) 0xef };
	final static long tv_p1 = 0x305532286d6f295aL;
	final static long tv_c1 = 0x55cb3774d13ef201L;

	// test vector #2 (offical vector by Bruce Schneier)
	final static String TESTKEY2 = "Who is John Galt?";
	final static long tv_p2 = 0xfedcba9876543210L;
	final static long tv_c2 = 0xcc91732b8022f684L;
	...
	public static boolean selfTest() {
		byte[] tv_p1_b = new byte[8];
		byte[] tv_t1_b = new byte[8];
		byte[] tv_p2_b = new byte[8];
		byte[] tv_t2_b = new byte[8];
		BlowfishECB testbf1;
		BlowfishECB testbf2;
		...
		BinConverter.longToByteArray(tv_p1, tv_p1_b, 0);
		testbf1.encrypt(tv_p1_b, 0, tv_t1_b, 0, tv_p1_b.length);

		if (tv_c1 != BinConverter.byteArrayToLong(tv_t1_b, 0)) {
			return false;
		}

		testbf1.decrypt(tv_t1_b, 0, tv_t1_b, 0, tv_t1_b.length);

		if (tv_p1 != BinConverter.byteArrayToLong(tv_t1_b, 0)) {
			return false;
		}

		testbf2 = new BlowfishECB(TESTKEY2.getBytes(), 0, TESTKEY2.length());

		BinConverter.longToByteArray(tv_p2, tv_p2_b, 0);
		testbf2.encrypt(tv_p2_b, 0, tv_t2_b, 0, tv_p2_b.length);

		if (tv_c2 != BinConverter.byteArrayToLong(tv_t2_b, 0)) {
			return false;
		}

		testbf2.decrypt(tv_t2_b, 0, tv_t2_b, 0, tv_t2_b.length);

		if (tv_p2 != BinConverter.byteArrayToLong(tv_t2_b, 0)) {
			return false;
		}
	...
	public boolean weakKeyCheck() {
		...
		for (i = 0; i < SBOX_ENTRIES - 1; i++) {
			for (j = i + 1; j < SBOX_ENTRIES; j++) {
				if ((this.sbox1[i] == this.sbox1[j]) | (this.sbox2[i] == this.sbox2[j])
						| (this.sbox3[i] == this.sbox3[j]) | (this.sbox4[i] == this.sbox4[j])) {
					this.weak_key = 1;
					return true;
				}
			}
	...
	protected int encryptPrv(byte[] inbuf, int inpos, byte[] outbuf, int outpos, int len) {
		len -= len % BLOCKSIZE;
	...
	/**
	 * Encrypts a byte buffer to another buffer, extra misaligned data will not
	 * be processed.
	 * 
	 * @param inbuf
	 *            buffer with plaintext data
	 * @param inpos
	 *            where to start reading the plaintext data
	 * @param outbuf
	 *            buffer to put the ciphertext data
	 * @param outpos
	 *            where to start writing the ciphertext data
	 * @param len
	...
	/**
	 * Decrypts a byte buffer to another buffer, extra misaligned data will not
	 * be processed.
	 * 
	 * @param inbuf
	 *            buffer with ciphertext data
	 * @param inpos
	 *            where to start reading the ciphertext data
	 * @param outbuf
	 *            buffer to put the plaintext data
	 * @param outpos
	 *            where to start writing the plaintext data
	 * @param len
	...
	final static int PBOX_INIT[] = { 0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344, 0xa4093822, 0x299f31d0,
			0x082efa98, 0xec4e6c89, 0x452821e6, 0x38d01377, 0xbe5466cf, 0x34e90c6c, 0xc0ac29b7, 0xc97c50dd, 0x3f84d5b5,
			0xb5470917, 0x9216d5d9, 0x8979fb1b };

	final static int SBOX1_INIT[] = { 0xd1310ba6, 0x98dfb5ac, 0x2ffd72db, 0xd01adfb7, 0xb8e1afed, 0x6a267e96,
	...
	final static int SBOX2_INIT[] = { 0x4b7a70e9, 0xb5b32944, 0xdb75092e, 0xc4192623, 0xad6ea6b0, 0x49a7df7d,
	...
	final static int SBOX3_INIT[] = { 0xe93d5a68, 0x948140f7, 0xf64c261c, 0x94692934, 0x411520f7, 0x7602d4f7,
	...
	final static int SBOX4_INIT[] = { 0x3a39ce37, 0xd3faf5cf, 0xabc27737, 0x5ac52d1b, 0x5cb0679e, 0x4fa33742,
