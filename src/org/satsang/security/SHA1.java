...
public class SHA1 {
	...
	final int blk0(int i) {
		return this.block[i] = ((rol(this.block[i], 24) & 0xff00ff00) | (rol(this.block[i], 8) & 0x00ff00ff));
	}

	// /////////////////////////////////////////////////////////////////////////

	final int blk(int i) {
		return (this.block[i & 15] = rol(this.block[(i + 13) & 15] ^ this.block[(i + 8) & 15]
				^ this.block[(i + 2) & 15] ^ this.block[i & 15], 1));
	}
	...
	public void finalize() {
		...
		for (i = 0; i < 20; i++) {
			this.digest_bits[i] = (byte) ((this.state[i >> 2] >> ((3 - (i & 3)) << 3)) & 0xff);
		}
	}
	 ...
	 * 
	 * @param buf
	 *            buffer to store the digst into
	 * @param ofs
	 *            where to write to
	 * @return number of bytes written
	 */
	public int getDigest(byte[] buf, int ofs) {
	...
	private final static byte[] SELFTEST_DIGEST = { (byte) 0x84, (byte) 0x98, (byte) 0x3e, (byte) 0x44, (byte) 0x1c,
			(byte) 0x3b, (byte) 0xd2, (byte) 0x6e, (byte) 0xba, (byte) 0xae, (byte) 0x4a, (byte) 0xa1, (byte) 0xf9,
			(byte) 0x51, (byte) 0x29, (byte) 0xe5, (byte) 0xe5, (byte) 0x46, (byte) 0x70, (byte) 0xf1 };
	...
	public boolean selfTest() {
		...
		tester.update(SELFTEST_MESSAGE.getBytes(), 0, SELFTEST_MESSAGE.length());
		tester.finalize();
